...
class CreatePermissionValidator {
  ...
  public messages = {
    ...
    'data.*.name.unique': 'Já existe uma permissão com este nome.',
...
class UpdatePermissionValidator {
  ...
  public schema = schema.create({
    data: schema.array().members(
      schema.object().members({
        id: schema.string({ trim: true }, [rules.exists({ table: 'permissions', column: 'id' })]),
  ...
  public messages = {
    ...
    'data.*.id.exists': 'A permissão especificada não existe.',
    'data.*.name.string': 'O campo "name" deve ser uma string válida.',
    'data.*.name.unique': 'Já existe uma permissão com este nome.',
    'data.*.description.string': 'O campo "description" deve ser uma string válida.',
  };
